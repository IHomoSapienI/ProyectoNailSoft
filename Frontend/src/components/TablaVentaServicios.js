            ...
            <div className="w-full ">
                <table className="min-w-full divide-y divide-gray-200 bg-white border border-gray-300 rounded-lg shadow-md table">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">ID Venta</th>
                            <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Cliente</th>
                            <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Documento</th>
                            <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha y Hora de Venta</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {ventas.slice((paginaActual - 1) * ventasPorPagina, paginaActual * ventasPorPagina).map((venta) => (
                            <tr key={venta._id}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta._id}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{venta.cliente?.nombrecliente}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta.cliente?.documentocliente}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta.estado ? 'Activo' : 'Inactivo'}</td>
            ...
            <div className="mt-4">
                <nav className="flex justify-center">
                    <ul className="inline-flex items-center">
                        ...
                        <li>
                            <button
                                onClick={() => setPaginaActual(prev => Math.min(prev + 1, Math.ceil(ventas.length / ventasPorPagina)))}
                                disabled={paginaActual === Math.ceil(ventas.length / ventasPorPagina)}
                                className={`px-3 py-1 mx-1 rounded ${
                                    paginaActual === Math.ceil(ventas.length / ventasPorPagina) ? 'bg-gray-200 text-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                }`}
                            >
                                &gt;
                            </button>
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg max-w-4xl w-full relative">
                    ...
                    {detallesCita && (
                        <div className="space-y-4">
                            <p><strong>ID Venta:</strong> {detallesCita._id}</p>
                            <p><strong>Cliente:</strong> {detallesCita.cliente.nombrecliente}</p>
                            <p><strong>Documento:</strong> {detallesCita.cliente?.documentocliente || 'Documento no disponible'}</p>
                            
                            <p><strong>Fecha y Hora de Venta:</strong> {formatearFechaHora(detallesCita.fecha)}</p>
                            <p><strong>Fecha y Hora de Cita:</strong> {formatearFechaHora(detallesCita.cita.fechacita)}</p>
                            <p><strong>Empleado:</strong> {detallesCita.empleado?.nombreempleado || 'Empleado no disponible'}</p>
                            <div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200 mt-2">
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${detallesCita.precioTotal.toFixed(2)}</td>
                            ...
                            <p><strong>Estado:</strong> {detallesCita.estado ? 'Activo' : 'Inactivo'}</p>
                        </div>
                    )}
