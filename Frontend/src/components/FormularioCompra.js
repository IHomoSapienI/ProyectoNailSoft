            ...
            try {
                ...
                if (compra) {
                    setFormData({
                        ...
                        insumos: compra.insumos.map(insumo => ({
                            _id: insumo.insumo._id,
                            nombreInsumo: insumo.insumo.nombreInsumo,
    ...
    const handleInsumoChange = (id, field, value) => {
        setFormData(prevState => ({
            ...
            insumos: prevState.insumos.map(insumo =>
                insumo._id === id ? { ...insumo, [field]: parseInt(value) || 0 } : insumo
            )
        }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.status === 401) {
                ...
                Swal.fire('Error', `Error en los datos enviados: ${error.response.data.message || 'Verifique los campos e intente nuevamente.'}`, 'error');
            } else {
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl overflow-auto">
                <h2 className="text-2xl font-semibold mb-6 text-center">
                    {compra ? 'Editar Compra' : 'Agregar Compra'}
                </h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                {proveedores.map((prov) => (
                                    <option key={prov._id} value={prov._id}>{prov.nombreProveedor}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        <label htmlFor="insumo" className="block mb-1">Insumos</label>
                        <div className="flex space-x-2">
                            <select
                                id="insumo"
                                value={nuevoInsumo.id}
                                onChange={(e) => setNuevoInsumo({ id: e.target.value, nombre: insumos.find(i => i._id === e.target.value)?.nombreInsumo || '' })}
                                className="w-full p-2 border rounded"
                            >
                                <option value="">Selecciona un insumo</option>
                                {insumos.map(insumo => (
                                    <option key={insumo._id} value={insumo._id}>{insumo.nombreInsumo}</option>
                                ))}
                            </select>
                            <button
                                type="button"
