...
const FormularioVentaServicio = ({ venta, onGuardar, onCancelar }) => {
    ...
    useEffect(() => {
        if (venta) {
            ...
            setServiciosSeleccionados(venta.servicios.map(servicio => ({
    ...
    return (
        <form onSubmit={manejarSubmit} className="space-y-4">
            <h2 className="text-xl font-semibold mb-4">{venta ? 'Editar Venta de Servicio' : 'Agregar Venta de Servicio'}</h2>

            {ventaId && (
                <div>
                    <label htmlFor="ventaId" className="block mb-1">ID de la Venta</label>
                    <input id="ventaId" value={ventaId} readOnly className="w-full p-2 border rounded bg-gray-100" />
                </div>
            )}

            <div>
                <label htmlFor="fecha" className="block mb-1">Fecha de Venta</label>
                <input id="fecha" value={fecha} readOnly className="w-full p-2 border rounded bg-gray-100" />
            </div>

            <div>
                <label htmlFor="cliente" className="block mb-1">Cliente</label>
                <select
                    id="cliente"
                    value={clienteId}
            ...
            <div>
                <label htmlFor="documento" className="block mb-1">Documento del Cliente</label>
                <input id="documento" value={clienteDocumento} readOnly className="w-full p-2 border rounded bg-gray-100" />
            </div>

            <div>
                <label htmlFor="cita" className="block mb-1">Cita</label>
                <select
                    id="cita"
                    value={citaId}
            ...
            <div>
                <label htmlFor="empleado" className="block mb-1">Empleado</label>
                <select
                    id="empleado"
                    value={empleadoId}
            ...
            <div>
                <label htmlFor="servicio" className="block mb-1">Servicios</label>
                <select
                    id="servicio"
                    value={nuevoServicio.id}
                    onChange={(e) => setNuevoServicio({ id: e.target.value, nombre: servicios.find(s => s._id === e.target.value)?.nombreServicio || '' })}
                    className="w-full p-2 border rounded"
                >
            ...
            <button type="submit" className="p-2 bg-green-500 text-white rounded">Guardar Venta</button>
                <button type="button" onClick={onCancelar} className="p-2 bg-gray-300 rounded">Cancelar</button>
